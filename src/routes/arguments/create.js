var ClaimModel = require("../../queries/claim-model.js");
var ArgumentModel = require("../../queries/argument-model.js");
var PremiseLinks = require("../../queries/premise-link-model.js");
...
module.exports = function create(req, res) {
  ...
  if (
    !req.body.hasOwnProperty("parentClaimId") ||
    req.body.parentClaimId == ""
  ) {
  ...
  ClaimModel.getById(parentClaimId)
    ...
    .then(existingArguments => {
      ...
      for (let a = 0; a < existingArguments.length; a++) {
        ...
        linkPromises.push(
          PremiseLinks.getUsedInEdges("_to", existingArguments[a]._id)
        );
      }
    ...
    .then(edges => {
      ...
      for (var a = 0; a < returnParentClaimObject.arguments.length; a++) {
        ...
        for (var l = 0; l < edgesPointingToArguments.length; l++) {
          if (
            edgesPointingToArguments[l]._to ==
            returnParentClaimObject.arguments[a]._id
          ) {
    ...
    .then(premiseObjects => {
      if (premiseObjects != "Parent claim has no args") {
        ...
        if (
          premiseObjects.length > 0 &&
          returnParentClaimObject.hasOwnProperty("arguments")
        ) {
          for (var a = 0; a < returnParentClaimObject.arguments.length; a++) {
            //now go through the arguemtn premises (they only have their ids at the moment)
            for (
              var pid = 0;
              pid < returnParentClaimObject.arguments[a].premises.length;
              pid++
            ) {
              //pid for premise id
              //now we're trying to fill returnParentClaimObject.arguments[a].premises[pid] with real data
              for (let po = 0; po < premiseObjects.length; po++) {
                //po for premise object
                if (
                  premiseObjects[po]._id ==
                  returnParentClaimObject.arguments[a].premises[pid]._id
                ) {
                  returnParentClaimObject.arguments[a].premises[pid] =
                    premiseObjects[po];
                }
              }
            }
        ...
        if (returnParentClaimObject.hasOwnProperty("arguments")) {
          for (var a = 0; a < returnParentClaimObject.arguments.length; a++) {
            if (
              Utils.doArgumentsMatch(
                returnParentClaimObject.arguments[a],
                newArgument
              )
            ) {
              res.send({
                ...
                errors: [
                  { title: "Argument already exists for this parent claim" }
                ]
              });
    ...
    .then(newArgumentPreises => {
      ...
      newArgument.probability = ProbabilityCalculator.getArgumentProbability(
        newArgument.premises
      );
    ...
    .then(newArgumentNode => {
      ...
      return PremiseLinks.createArgumentEdge(
        newArgumentNode._id,
        returnParentClaimObject._id,
        newArgumentNode.type
      );
    })
    .then(newEdge => {
      ...
      for (var p = 0; p < newArgument.premises.length; p++) {
        promises.push(
          PremiseLinks.createUsedInEdge(
            newArgument.premises[p]._id,
            newArgument._id
          )
        );
      }
    ...
    .then(newUsedInEdges => {
      ...
      let newClaimProbability = ProbabilityCalculator.getClaimProbability(
        returnParentClaimObject.arguments
      );
      //set it in the return object
      returnParentClaimObject.probability = newClaimProbability;
      //and save it to the db
      return ClaimModel.updateProbability(
        returnParentClaimObject._id,
        newClaimProbability
      );
    })
