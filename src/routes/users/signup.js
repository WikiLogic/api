...
try {
  ...
  console.log(
    "To log in you're going to need to whitelist yourself with a guestlist.js file"
  );
...
module.exports = function signUp(req, res) {
  ...
  if (!whitelisted) {
    res.json({
      message:
        "We're not open to public sign ups yet, please contact us to sign up"
    });
    return;
  }
  ...
  Users.checkIfUnique({
    ...
    .then(isUnique => {
      if (!isUnique) {
        res.json({
          errors: [{ title: "Duplicate credentials" }]
        });
        return;
      }
      ...
      Users.createUser(email, username, hash)
        .then(newUser => {
          ...
          var token = jwtService.jwtSign(
            payload,
            jwtService.jwtOptions.secretOrKey
          );
