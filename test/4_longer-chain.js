...
describe("Testing a slightly longer chain", function() {
  ...
  it("Create Claim1", function(done) {
    api
      .post("/claims")
      .send({
        text: testData.claim1.text,
        probability: testData.claim1.probability
      })
      .set("Accept", "application/json")
  ...
  it("Create Claim2", function(done) {
    api
      .post("/claims")
      .send({
        text: testData.claim2.text,
        probability: testData.claim2.probability
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
  ...
  it("Create Claim3", function(done) {
    api
      .post("/claims")
      .send({
        text: testData.claim3.text,
        probability: testData.claim3.probability
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
  ...
  it("Create Argument1", function(done) {
    api
      ...
      .then(response => {
        ...
        assert(
          apiData.claim1._id == response.body.data.claim._id,
          "The returned claim should be an updated claim 1"
        );
        assert(
          response.body.data.claim.arguments.length == 1,
          "the updated claim 1 should now have one argument"
        );
        apiData.claim1 = response.body.data.claim;
        done();
      });
  ...
  it("Create Argument2", function(done) {
    api
      ...
      .then(response => {
        ...
        assert(
          apiData.claim2._id == response.body.data.claim._id,
          "The returned claim should be an updated claim 1"
        );
        assert(
          response.body.data.claim.arguments.length == 1,
          "the updated claim 2 should now have one argument"
  ...
  it("Getting claim1 should return claim1 with the 1 argument we added", function(done) {
    api
      .get(`/claims/${apiData.claim1._key}`)
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
      .then(response => {
        assert(
          response.body.data.claim._id == apiData.claim1._id,
          "returned claim should have an _id"
        );
        assert(
          response.body.data.claim._key == apiData.claim1._key,
          "returned claim should have a _key"
        );
        assert(
          response.body.data.claim.arguments.length == 1,
          "Getting Claim 1 should now return with 1 argument"
        );
        done();
      });
  });
  it("Getting claim2 should return claim2 with the 1 argument we added", function(done) {
    api
      .get(`/claims/${apiData.claim2._key}`)
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
      .then(response => {
        assert(
          response.body.data.claim._id == apiData.claim2._id,
          "returned claim should have an _id"
        );
        assert(
          response.body.data.claim._key == apiData.claim2._key,
          "returned claim should have a _key"
        );
        assert(
          response.body.data.claim.arguments.length == 1,
          "Getting Claim 2 should now return with 1 argument"
        );
        done();
      });
  });
  ...
  it("Delete argument1", function(done) {
    api
      .del("/arguments")
      .send({ _id: apiData.claim1.arguments[0]._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
  it("Delete argument2", function(done) {
    api
      .del("/arguments")
      .send({ _id: apiData.claim2.arguments[0]._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
