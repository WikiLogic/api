...
describe("Testing basic Arguments", function() {
  ...
  it("Creating a new claim for the sake of argument testing, should work", function(done) {
    api
      .post("/claims")
      .send({
        text: srcArgTestClaims.top.text,
        probability: srcArgTestClaims.top.probability
      })
      .set("Accept", "application/json")
      ...
      .then(response => {
        assert(
          response.body.data.claim._id,
          "returned claim should have an _id"
        );
        assert(
          response.body.data.claim._key,
          "returned claim should have a _key"
        );
  ...
  it("Creating a new claim for the sake of argument testing, should work", function(done) {
    api
      .post("/claims")
      .send({
        text: srcArgTestClaims.against1.text,
        probability: srcArgTestClaims.against1.probability
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
  ...
  it("Creating a new claim for the sake of argument testing, should work", function(done) {
    api
      .post("/claims")
      .send({
        text: srcArgTestClaims.against2.text,
        probability: srcArgTestClaims.against2.probability
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
  ...
  it("Creating a new claim for the sake of argument testing, should work", function(done) {
    api
      .post("/claims")
      .send({
        text: srcArgTestClaims.for1.text,
        probability: srcArgTestClaims.for1.probability
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
  ...
  it("Creating a new claim for the sake of argument testing, should work", function(done) {
    api
      .post("/claims")
      .send({
        text: srcArgTestClaims.for2.text,
        probability: srcArgTestClaims.for2.probability
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
  ...
  it("new FOR argument to the top test claim, should return updated top test claim", function(done) {
    api
      ...
      .then(response => {
        ...
        assert(
          response.body.data.claim._id == argTestClaims.top._id,
          "The parent claim should be the one returned"
        );
        assert(
          returnedArgs.length == 1,
          "The claim should now have 1 argument"
        );
        assert(
          response.body.data.claim.probability,
          "The claim should have a probability"
        );
        assert(
          returnedArgs[0].type == "FOR",
          "The returned argument in the parent claim should be FOR"
        );

        for (var a = 0; a < returnedArgs.length; a++) {
          returnedArgs[a];
          assert(
            returnedArgs[a].probability,
            "each argument should have a probability set"
          );
        }
  ...
  it("Getting the top claim should be returned with the argument we just created", function(done) {
    api
      .get(`/claims/${argTestClaims.top._key}`)
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
      .then(response => {
        ...
        assert(
          response.body.data.claim._key == argTestClaims.top._key,
          "returned claim should have a _key"
        );
        assert(
          response.body.data.claim.arguments.length == 1,
          "The top claim should now have the one argument we just added" +
            JSON.stringify(response.body.data.claim, null, 2)
        );
        assert(
          response.body.data.claim.probability,
          "The claim should have a probability"
  ...
  it("new AGAINST argument to the top test claim, should return updated top test claim", function(done) {
    api
      ...
      .then(response => {
        ...
        assert(
          response.body.data.claim.arguments.length == 2,
          "The claim should now have 2 arguments" +
            JSON.stringify(response.body.data.claim, null, 2)
        );
        assert(
          response.body.data.claim.probability,
        ...
        for (var a = 0; a < returnedArgs.length; a++) {
          assert(
            returnedArgs[a].probability,
            "each returned argument should have a probability"
          );
          assert(
            returnedArgs[a]._id,
            "each returned argument should have an _id property"
          );
          assert(
            returnedArgs[a]._id.indexOf("/") > 0,
            "the _id property should match the arango format"
          );
          assert(
            returnedArgs[a]._key,
            "each returned argument should have a _key property"
          );
          assert(
            returnedArgs[a]._key.indexOf("/") == -1,
            "the _key property should match the arango format"
          );
          assert(
            returnedArgs[a].type,
            "each returned argument should have a type property"
          );
          assert(
            returnedArgs[a].creationDate,
            "each returned argument should have a creationDate property"
          );
        }
  ...
  it("Getting the top claim should be returned with both the new arguments", function(done) {
    api
      ...
      .then(response => {
        ...
        assert(
          response.body.data.claim.arguments.length == 2,
          "The top claim should now have both the arguments we just added"
        );
        assert(
          response.body.data.claim.probability,
          "The claim should have a probability"
  ...
  it("Getting the parent claim Should return the claim with populated argument data", function(done) {
    api
      ...
      .then(response => {
        assert(
          response.body.data.claim.text == argTestClaims.top.text,
          "Returned claim should have the text we're expecting"
        );
        assert(
          response.body.data.claim._id == argTestClaims.top._id,
          "Returned claim should have the id we're expecting"
        );
        assert(
          response.body.data.claim.arguments.length == 2,
          "Returned claim should have the the 2 arguments we added"
        );
        assert(
          response.body.data.claim.probability,
          "The claim should have a probability"
        ...
        assert(
          response.body.data.claim.arguments[0].hasOwnProperty("premises"),
          "The arguments in the claim should have an array of premises"
        );
        assert(
          response.body.data.claim.arguments[0].premises[0].hasOwnProperty(
            "_id"
          ),
          "The premises in the argument should be populated"
        );
  ...
  it("Delete for argument", function(done) {
    api
      .del("/arguments")
      .send({ _id: argTestClaims.arguments[0]._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
  ...
  it("Delete against argument", function(done) {
    api
      .del("/arguments")
      .send({ _id: argTestClaims.arguments[1]._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
  ...
  it("Delete top claim", function(done) {
    api
      .del("/claims")
      .send({ _id: argTestClaims.top._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
  it("Delete against 1", function(done) {
    api
      .del("/claims")
      .send({ _id: argTestClaims.against1._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
  it("Delete against 1", function(done) {
    api
      .del("/claims")
      .send({ _id: argTestClaims.against2._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
  it("Delete for 1", function(done) {
    api
      .del("/claims")
      .send({ _id: argTestClaims.for1._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
  it("Delete for 2", function(done) {
    api
      .del("/claims")
      .send({ _id: argTestClaims.for2._id })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200, done);
  });
