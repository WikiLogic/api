...
describe("Testing basic Claims", function() {
    ...
    api
      ...
      .then(response => {
        ...
        assert.exists(
          ...
          "Returned new claim should have a ._id"
        );
        assert.exists(
          response.body.data.claim._key,
          "Returned new claim should have a ._key"
        );
    ...
    api
      ...
      .then(response => {
        ...
        assert(
          ...
          "Trying to create a claim that already exists should prompt an error in the api response"
        );
        done();
      });
  });
  ...
  it("Posting claim with similar text should return a new claim, not the existing one", function(done) {
    api
      .post("/claims")
      .send({
        text: srcSimilarTestClaim.text,
        probability: srcSimilarTestClaim.probability
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
      .then(response => {
        assert(
          response.body.data.claim.text == srcSimilarTestClaim.text,
          "Returned new claim should have the text we set"
        );
        assert.exists(
          response.body.data.claim._id,
  ...
  it("Posting claim with undefined probability should return a new claim presuming 0.5 probability", function(done) {
    api
      .post("/claims")
      .send({
        text: srcUndefinedProbabilityTestClaim.text,
        probability: undefined
      })
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      ...
      .then(response => {
        assert(
          response.body.data.claim.text ==
            srcUndefinedProbabilityTestClaim.text,
          "Returned new claim should have the text we set"
        );
        assert.exists(
          response.body.data.claim._id,
  ...
  it("Getting the claim we just created by it's id Should return that claim", function(done) {
    api
      ...
      .then(response => {
        ...
        assert(
          response.body.data.claim.probability == srcTestClaim.probability,
          "Returned claim should still have the initial probability we set"
        );
        assert(
          response.body.data.claim._id == testClaim._id,
  ...
  it("Full text search for the claim we just created should return the claim", function(done) {
    api
      ...
      .then(response => {
        assert(
          response.body.data.results.length > 0,
          "The search should return at least one thing"
        );
  ...
  it("Partial text search for the claim we just created should return the claim", function(done) {
    api
      .get(`/claims/search?s=${srcTestClaim.searchText}`)
      .set("Accept", "application/json")
      .set("Authorization", JWT)
      .expect(200)
      .then(response => {
